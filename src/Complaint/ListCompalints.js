...
const ListComplaint = ({navigation,route}) => {
...
/* States For display array in dropdown */
  ...
  const getComplains = () =>{
    ...
    setLoading(true)
    fetch(`${baseUrl}/complains`, {
      method: 'GET',
    })
    .then(resp => resp.json())
    .then(respComplain => {

      // console.log('ALl Complains', respComplain)
      // tableData: [
      //   ['1', '2', '3'],
      //   ['a', 'b', 'c'],
      //   ['1', '2', '3'],
      ...
      var count = Object.keys(respComplain.complains).length;
      // console.log('COunt', count)
      let complinData = [];
      for (var i = 0; i < count; i++) {
        const createdAtDate = respComplain.complains[i].created_at;
        const updatedAtDate = respComplain.complains[i].updated_at;
        // // const updatedAtTime = respComplain.complains[i].updated_at.getTime() / 1000
        ...
        complinData.push([`PMACID-${respComplain.complains[i].id}`,`${respComplain.complains[i].complaint_details}`,`${respComplain.complains[i].complaint_type}`,createdAtDate.toString().substr(0 ,10),
        ...
        <View value={orientation} style={{width:600,opacity:1, backgroundColor:'#fff', height:'100%',padding:10, borderRadius:10}}>
          <ScrollView>
            <View style={{flex:1, backgroundColor:'#fff',alignSelf:'center',}}>
              <Text style={[styles.text2,{fontSize:25, marginBottom:10,color:'black'}]}>List Of Complains</Text>
            </View>
            <View style={{flex:1,backgroundColor:'red', flexDirection:'row',justifyContent:'space-evenly',width:'80%',}}>
              <View style={{alignItems: 'center'}}>
                <TouchableOpacity
                  // onPress={onLoginPress}
                  style={[styles.ButtonStyle,{padding:10,backgroundColor:'black',borderRadius:10,width:'100%'}]}
                  activeOpacity={0.5}>
                  <Text style={[styles.text, { textAlign: 'center' }]}>Resolved</Text>
                </TouchableOpacity>
            ...
            <View  style={{width:'100%',opacity:0.9, backgroundColor:'#fff',alignSelf:'center', height:'100%',paddingLeft:50, borderRadius:30}}>
              ...
              <Table borderStyle={{borderWidth: 0, borderColor: '#444',}}>
                <Row data={['Complain ID','Description','Complain Type', 'Complain Date', 'Action']} style={styles.head} textStyle={styles.text}/>
...
const styles = StyleSheet.create({
  ...
  row: { height: 80, backgroundColor: '#fff', color:'#444',width:'90%' },
