...
const CitiesComplaintresolved = ({ navigation, route }) => {
  ...
  const fetchArchivedCount = async () => {
    try {
      const response = await fetch(`https://complaint-pma.punjab.gov.pk/api/archivedCount/total/${route.params.data[0]['cityIds']}`);
...
const renderRightActions = (progress, dragX, item) => {
    ...
    return (
      <TouchableOpacity 
        onPress={() => isArchived ? handleRestore(item.id) : handleArchive(item.id, item.status)}
        style={styles.rightAction}
      >
        <FontAwesomeIcon 
          name={isArchived ? "refresh" : "archive"} 
          size={15} 
          color={isArchived ? "maroon" : "maroon"} 
...
//           item.archive === 1 && styles.archivedItem, // Apply light gray color if archived
//         ]}
//       >
...
//         {/* Render "Archived" text if the item is archived */}
//         {item.archive === 1 && (
    ...
    >
      <TouchableOpacity
        ...
        style={[
          styles.collapsibleHeader,
          getStatusBorderColor(item.status),
          { borderLeftColor: item.archive == 1 ? 'maroon' : 'green' } // Change color for archived items
        ]}
      >
        <View style={styles.collapheaderItem}>
          <Text style={[
          styles.collapheaderText,
          getStatusBorderColor(item.status),
          { color: item.archive == 1 ? 'maroon' : 'black' } // Change color for archived items
        ]}
      >Complaint ID: {item.id}</Text>
          <Text  style={[
          styles.statusText,
          getStatusBorderColor(item.status),
          { color: item.archive == 1 ? 'maroon' : 'green' } // Change color for archived items
        ]}
      >Status: {item.status}</Text>
          <Icon name={collapsedStates[index] ? 'chevron-down' : 'chevron-up'} size={12} color="#666" />
        </View>
        ...
        <Collapsible collapsed={collapsedStates[index]}>
          <View style={styles.collapsibleContent}>
            <View style={styles.rowContainer}>
              <Text style={styles.labelText}>
                Complaint Type: <Text style={styles.innerlabelText}>{item.complaint_type}</Text>
              </Text>
            </View>
            <View style={styles.rowContainer}>
              <Text style={styles.labelText}>
                Complaint Details: <Text style={styles.innerlabelText}>{item.complaint_details}</Text>
              </Text>
            </View>
            <View style={styles.rowContainer}>
              <Text style={styles.labelText}>
                Service Provider: <Text style={styles.innerlabelText}>{item.service_provider_name}</Text>
              </Text>
            </View>
            <View style={styles.rowContainer}>
              <Text style={styles.labelText}>Picture: {item.complain_pic}</Text>
            </View>
          </View>
        </Collapsible>
      </TouchableOpacity>
  ...
  const handleRestore = (id) => {
    ...
    .then(async data => { // Using async to call fetchData and fetchArchivedCount sequentially
      if (data.message) {
        ...
        await fetchData(); // Reload the list after restoring
...
const styles = StyleSheet.create({
  ...
  archivedItem: {
    backgroundColor: 'lightgray', // Color for archived items
  },
  archivedText: {
    color: 'maroon', // Color for "Archived" text
    fontWeight: 'bold',
    textAlign: 'center',
    fontStyle: 'italic',
    fontSize: 6,
...
export default CitiesComplaintresolved;
