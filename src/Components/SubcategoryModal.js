...
const SubcategoryVerified = ({ visible, onClose, categoryId, userId }) => {
  ...
  const fetchSubcategories = async () => {
    try {
      setLoading(true);
      const response = await fetch(`https://complaint-pma.punjab.gov.pk/api/categorywisedataddacat/${userId}/${categoryId}`);
  ...
  const handleSubcategoryPress = (subcategoryId, subcategoryName) => {
    navigation.navigate('Cccacat', { subcategoryId, categoryId, subcategoryName });
  };
