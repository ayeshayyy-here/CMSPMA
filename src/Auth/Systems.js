...
const SystemScreen = ({route, navigation}) => {
  ...
  const fetchSystemWiseHierarchy = providerId => {
    fetch(
      `https://complaint-pma.punjab.gov.pk/api/systemwisehierarchy/${providerId}`,
    )
      .then(response => response.json())
      .then(data => {
  ...
  const fetchData2 = async () => {
    try {
      const response = await fetch(`${baseUrl}/designationss`); // Corrected the URL
  ...
  const fetchDesignationStaffData2 = async () => {
    try {
      const designationId = 2; // Static designation id for General Manager
      const response = await fetch(
        `${baseUrl}/designationpmastaff/${designationId}`,
      );
      const data = await response.json();
      console.log("Fetched General Manager's staff data:", data);
      setDesignationStaffData2(data);
    } catch (error) {
      console.error('Error fetching General Manager staff data:', error);
    }
  };
  ...
  const renderContent4 = () => {
    return (
      <TouchableOpacity
        onPress={() => {
          fetchDesignationStaffData2(); // Fetch General Manager's staff data
        }}
        activeOpacity={0.5} // Change opacity when pressed
      >
        <LinearGradient
          colors={[
            selectedDesignation === 'General Manager'
              ? 'rgba(255, 218, 185, 0.5)'
              : 'rgba(255, 182, 193, 0)', // Apply different color if selected
            'rgba(255, 218, 185, 0)',
          ]}
          start={{x: 0, y: 0}}
          end={{x: 1, y: 1}}
          style={[
            styles.scrollItem,
            selectedDesignation === 'General Manager' &&
              styles.selectedScrollItem, // Apply selected style if designation is selected
          ]}>
          <Text
            style={[
              styles.scrollText,
              selectedDesignation === 'General Manager' && {color: '#000000'}, // Apply different text color if selected
            ]}>
            General Manager
          </Text>
        </LinearGradient>
  ...
  const renderContentHierarchy = () => {
    ...
    const selectedDesignationName = 'General Manager';
    const colors = ['#8b0000', '#7d0000', '#6f0000', '#610000', '#530000'];
        ...
        <View
          ...
          <View style={styles.userDetailsGrid}>
            {designationStaffData2.map(
              (
                user,
                index, // Added arrow function syntax
              ) => (
                <View style={styles.userDetailsContainer} key={index}>
                  <TouchableOpacity
                    key={user.id}
      ...
      <ScrollView style={{marginBottom: '20%'}}>
              ...
              <View style={styles.systemWiseHierarchyContainer}>
                {systemWiseHierarchy.map(item => (
                  <TouchableOpacity
                    key={item.id}
                    onPress={() => {
                      navigation.navigate('SystemWiseComplaint', {
                        itemId: item.id,
                        itemName: item.name,
                      });
                    }}
                    ...
                    <View style={styles.profileImageContainerr}>
                      <Text style={styles.initialSystemwise}>
        ...
        <View style={{flex: 1}}>
          <TouchableOpacity
            ...
            onPress={handleToggleCollapse2}>
            <Text style={{fontSize: 15, fontWeight: 'bold', color: 'black'}}>
              Designations Wise staff
            </Text>
            <Icon
              name={collapsed2 ? 'chevron-down' : 'chevron-up'}
              size={10}
              color="gray"
            />
          </TouchableOpacity>
        ...
        <View style={{flex: 1}}>
          <TouchableOpacity
            ...
            onPress={handleToggleCollapse3}>
            <Text style={{fontSize: 15, fontWeight: 'bold', color: 'black'}}>
              Hierarchy Wise staff
            </Text>
            <Icon
              name={collapsed3 ? 'chevron-down' : 'chevron-up'}
              size={10}
              color="gray"
            />
          </TouchableOpacity>
...
const styles = StyleSheet.create({
  line: {
    height: 1, // or any height you need
    backgroundColor: '#D3D3D3',
    marginVertical: 15, // optional, to add some vertical spacing
    marginHorizontal: 20,
  },
  noDataContainer: {
    flex: 1,
