...
const Sourcedetail = ({ navigation, route }) => {
  ...
  useEffect(() => {
    const source = route.params.complaintsData.source;
    setHeaderComplain(source);
    fetchData(0);
  }, []);
  const fetchArchivedCount = async () => {
    try {
      const source = route.params.complaintsData.source;
      const response = await fetch(`https://complaint-pma.punjab.gov.pk/api/Archivecountsource/${source}`);
  ...
  const reloadData = async () => {
    setLoadingData(true);
    await fetchData(0); // Reload the list
  ...
  const fetchData = async (page) => {
    ...
    try {
      const source = route.params.complaintsData.source;
      const response = await fetch(`https://complaint-pma.punjab.gov.pk/api/complainsss/${source}?per_page=20&page=${page + 1}`);
      const responseData = await response.json();
  ...
  return (
    ...
    <View style={styles.header}>
      ...
      <Text style={styles.headerText}>
            {headerComplain === 'instagram'
              ? 'Instagram Complaints'
              : headerComplain === 'ccc'
                ? 'CCC Complaints'
                 : headerComplain === 'pfi'
                ? 'PFI Complaints'
                : headerComplain === 'facebook'
                  ? 'Facebook Complaints'
                  : `${headerComplain} Complaints`}
          </Text>
      <View style={styles.rightHeader}>
        <TouchableOpacity onPress={() => navigation.navigate('ArchivedComplaintsource', { data: route.params.complaintsData.source})}>
