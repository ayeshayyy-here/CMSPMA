...
const Networkassignedquery = ({navigation, route}) => {
  ...
  const fetchOptions = async () => {
    setLoading(true);
    try {
      const response = await fetch(`${baseUrl}/networkassigntolist`);
  ...
  const assignquery = async () => {
    ...
    const requestData = {
      ...
      audio: recordedAudio, 
      complaintFile: stateFunction.URI,
    };
  
    console.log('Request Data:', requestData); // Log the request data before making the request
  
    try {
      const response = await fetch(`${baseUrl}/networkstorequery`, {
        method: 'POST',
        headers: {
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView>
      <Text style={styles.heading}>New Query</Text>
...
export default Networkassignedquery;
